> **Source.** This is the getdesign.md "Apple" design analysis (`npx getdesign@latest add apple`,
> `templates/apple.md`), Copyright (c) 2026 VoltAgent, MIT License (full notice: [DESIGN.LICENSE](DESIGN.LICENSE)),
> reproduced verbatim from the front matter down to "Known Gaps".
> It is an independent analysis, not affiliated with or endorsed by Apple Inc.; Apple, SF Pro and
> related names are Apple's trademarks. How Cite applies it — and every value the analysis does not
> cover — is the last section, [Project Application (Cite)](#project-application-cite). The UI must
> follow that section; where it is silent, the analysis below applies.
...
## Project Application (Cite)

This section is the project's own; everything above is the upstream analysis, unchanged. Behaviour,
states, copy and accessibility are owned by [`EXPERIENCE.md`](EXPERIENCE.md); this section owns how
Cite looks. The owner approved the look on the static mockup
`.superpowers/sdd/2026-09-24-wiki-agent/mock/4-chat.html` (Apple theme) on 2026-09-25; the shipped
implementation is `src/aidd_chat/web/styles.css` (literals only in `:root`), `index.html`, `app.js`
and `markdown.js`.

### Decision: ChatGPT structure, Apple tokens

The analysis describes a marketing catalogue, not a conversation shell. Cite therefore takes its
**structure** from chatgpt.com (measured 2026-09-25) and its **values** from the analysis:

- Structure: a 260px sidebar (items 36px, icons 20px) that collapses to a 52px icon rail; a centred
  768px column; a pill composer that sits centred on the start screen and becomes a pinned dock in a
  conversation; answers without a bubble; the question in a right-aligned bubble; a "thinking" line
  above the answer; an action row and a sources pill under it.
- Values: `{colors.ink}` text, `{colors.canvas}` / `{colors.canvas-parchment}` surfaces,
  `{colors.hairline}` borders, `{colors.primary}` as the single interactive colour,
  `{colors.primary-focus}` for the focus ring, body 17px / 1.47 / -0.374px, the frosted
  `saturate(180%) blur(20px)` of the floating sticky bar for the dock, the press `scale(0.95)`,
  and no shadow on chrome.
- Brand: **Cite**, tagline "모든 답에 근거 문서를". The mark is a single-weight quote glyph in a 30px
  `{colors.ink}` rounded square (radius 9px); it sits with the name in the sidebar top, next to the
  collapse toggle. The main area has no title bar. Font: the system SF stack, then self-hosted
  Pretendard (Korean).
...
|---|---|---|---|
| `--bg` | #ffffff | `canvas` | page, active sidebar item, cards, pills |
| `--parchment` | #f5f5f7 | `canvas-parchment` | sidebar, user bubble, code, disabled send |
| `--ink` | #1d1d1f | `ink` | all body text, the mark |
| `--ink-2` | #333333 | `ink-muted-80` | secondary text: steps, notices, metadata, paths |
| `--ink-fine` | #7a7a7a | `ink-muted-48` | fine print only (see Accessibility) |
| `--line` | #e0e0e0 | `hairline` | cards, source card, step rail |
| `--control-line` | rgba(0,0,0,.08) | chip border | pill composer, pills, tags |
| `--accent` / `--on-accent` | #0066cc / #ffffff | `primary` / `on-primary` | send, stop, 재시도, source titles |
| `--focus` | #0071e3 | `primary-focus` | 2px focus ring |
| `--hover` | rgba(0,0,0,.04) | project | hover wash |
| `--dock` | rgba(255,255,255,.8) | floating sticky bar | composer dock, mobile top bar |
| `--warning` | #b54708 | project (Known Gap) | 논쟁 중 / 신뢰도 낮음 flags |
| `--danger-line` / `--danger-soft` | #ecc8c5 / #fcf6f6 | project (Known Gap) | error card |
| `--scrim`, `--shadow-sheet` | | project | folded navigation Sheet only |
| `--font-mono` | ui-monospace stack | project | answer code only (`.md`) |

Type ramp: `--text-h1` 28px/600 (start question, policy title), `--text-body` 17px, `--text-card`
15px, `--text-label` 14px, `--text-control` 13px, `--text-caption` 12px, `--text-micro` 11px,
`--text-dot` 10px; Markdown `h3` 1.15em, inline code .875em. Spacing is the measured ChatGPT rhythm
on a 4px base with the half-steps it uses (2, 3, 6, 10, 14px). Radii: item 8, mark 9, row 10,
card 16, bubble and code 18, pill 9999; the composer pill is 28px, 24px once it wraps.

### Component mapping

| Cite component | Analysis source | Canonical ID |
|---|---|---|
| App shell: sidebar + column | parchment utility surface + white canvas | `UX-APP-CANVAS` |
| Sidebar (260px / 52px rail) and folded Sheet | global nav collapsing strategy | `UX-NAV-SHEET` |
| Pill composer | search input (pill) | `UX-COMPOSER` |
| Send circle (36px, blue) / 재시도 pill | `button-primary` | `UX-PRIMARY-ACTION` |
| Stop: the same circle showing a square | `button-primary` | `UX-STOP-ACTION` |
| Question bubble (parchment, radius 18) | `product-tile-parchment` surface | `UX-USER-MESSAGE` |
| Answer (no bubble, Markdown) | body typography | `UX-ASSISTANT-RESPONSE` |
| Thinking line + step list + run state | caption typography | `UX-STATUS-ANNOUNCER` |
| Error card (hairline, radius 16) | store utility card shape | `UX-RUN-RECOVERY` |
| Info lines, policy notice | caption typography | `UX-KNOWLEDGE-NOTICE` |
| Sources pill + source card | chip + store utility card | `UX-SOURCE-LIST` |
| 2px ring | `primary-focus` | `UX-FOCUS-RING` |
| Policy cards (hairline, radius 16) | store utility card | — |

### Accessibility decisions (WCAG 2.2 AA)

- Visible values stay the analysis's own. Body text is `--ink` (16.8:1 on white); every secondary
  text that carries information — steps, notices, metadata, source paths, the
  error card's details — is `--ink-2` (12.6:1), never the 48% grey.
- `#7a7a7a` (4.3:1 on white, 3.9:1 on parchment) is fine print only, as in the analysis ("legal
  fine-print"): the sidebar tagline, the 1-hour note, the composer placeholder, a disabled control's
  glyph and the dim ends of the thinking-line shimmer. `tests/test_story_1_10.py` enforces that list.
- Focus is `outline: 2px solid #0071e3` on every control (the composer draws it around the pill);
  at least 3:1 on every surface.
- Hairline boundaries (`--control-line` on the composer and pills) are the owner-approved look and
  are below 3:1; the composer is identified by its placeholder, its send circle and the focus ring,
  and every state is also text.
- Icon-only buttons (send, stop, copy, collapse, menu, close) carry an accessible name; send and
  stop also carry visually hidden text.

### Known-Gap fills

- **Error / recovery.** The analysis has no error state. The error card is a 16px-radius card with a
  faint red hairline (`--danger-line`) on `--danger-soft`: the message in `--ink`, the five recovery
  facts as `--ink-2` caption text, then 재시도 (blue pill when it is the primary action) and 새 대화.
  While the card sits under a failed answer it represents that answer alone (the text stays in the
  transcript for screen readers). Once the next question moves the card away, the failed answer shows
  again as a quiet line: 14px `--ink-2` behind a 2px `--danger-line` rail, so no question ever looks
  unanswered.
- **Warnings.** Document flags (논쟁 중, 신뢰도 낮음) are `--warning` text, never colour alone. The
  입력 금지 policy statement lives once, on the 정보·정책 screen (`data-policy-success`), not repeated
  under the composer.
- **Streaming marker.** Below a streaming answer, "생성 중인 답변입니다." in caption `--ink-2`
  with a drawn 2px caret that blinks (`ux-caret`).
- **Agent steps.** One thinking line above the answer: the current step label with a text shimmer
  while the Run is active; "Wiki N단계 확인 ›" once it settles; nothing for a settled Run without
  steps. Clicking expands the step list (2px `--line` rail on the left) and one caption,
  "<time> 갱신" (the last-update time); the Run state and stage are there for screen readers only.
  Earlier answers keep their own collapsed line.
- **Sources and outcome notices.** Under a completed answer: an action row with a copy icon button
  and, for grounded/partial, a "근거 문서 N" pill that opens a hairline card listing each source —
  title in `--accent` (text, not a link), Wiki-relative path in caption `--ink-2`, flags after the
  title. `partial` and search-limit notices are 14px `--ink-2` info lines with an info glyph;
  `wiki_gap` / `out_of_scope` are a small outlined tag ("Wiki 보충 대상" / "Wiki 범위 밖").
- **Markdown rendering rules.** Answers are parsed by `markdown.js` into an AST (CommonMark fences
  including longer/nested fences and tildes, inline code runs, `**bold**`, headings, lists,
  paragraphs) and built with `textContent` only: no HTML, no links, no images. Blocks are 14px
  apart; headings render at 1.15em/600; code blocks are parchment, radius 18, with a 40px header
  holding the language label and a "복사" ghost button ("복사됨" after copying); a fence still open
  while streaming renders as a code block without the button. The answer is re-parsed on every
  delta, but blocks that are already complete and unchanged stay in place (a closed code block keeps
  its button state and focus); the completed message is the source of truth.
- **Reduced motion.** Under `prefers-reduced-motion: reduce` every transition and animation
  (sidebar collapse, shimmer, press scale) is effectively instant; the caret stops blinking but
  stays drawn and the thinking line stops shimmering.

### Responsive

| Width | Cite |
|---|---|
| 1024px and up | 260px sidebar (collapsible to a 52px rail, remembered, Ctrl/Cmd+Shift+S) + 768px column |
| 768–1023px | folded navigation: a modal side Sheet (up to 320px) behind the menu trigger in a frosted top bar |
| below 768px | the same Sheet at full width; bubbles up to 85% |
